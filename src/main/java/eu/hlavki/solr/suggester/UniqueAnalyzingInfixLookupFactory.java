 ...
 * @lucene.experimental
 */
public class UniqueAnalyzingInfixLookupFactory extends LookupFactory {
    ...
    /**
     * Pattern for id field consist comination of letters (T,P,C,W).
     * T - text field value
     * P - payload field value
     * C - context field value (valid only if contexts field contains single value)
     * W - weight field value
     *
     * Example: TP - means text concatenated with payload value
     */
    public static final String ID_FIELD_PATTERN = "idValuePattern";

    private static final String DEFAULT_ID_FIELD_PATTERN = "T";
    ...
    public Lookup create(NamedList params, SolrCore core) {
        ...
        String idValuePattern = params.get(ID_FIELD_PATTERN) != null
                ? params.get(ID_FIELD_PATTERN).toString()
                : DEFAULT_ID_FIELD_PATTERN;

        try {
            return new UniqueAnalyzingInfixSuggester(FSDirectory.open(new File(indexPath).toPath()), indexAnalyzer,
                    queryAnalyzer, minPrefixChars, true,
                    allTermsRequired, highlight, idValuePattern) {
                @Override
                public List<Lookup.LookupResult> lookup(CharSequence key, Set<BytesRef> contexts, int num, boolean allTermsRequired, boolean doHighlight) throws IOException {
                    ...
                    if (doHighlight) {
                        List<Lookup.LookupResult> res2 = new ArrayList<>();
                        for (Lookup.LookupResult hit : res) {
                            res2.add(new Lookup.LookupResult(hit.highlightKey.toString(),
                                    hit.highlightKey,
                                    hit.value,
                                    hit.payload,
                                    hit.contexts));
